...

{
  ...
  services.smartdns = {
    ...
    # 也可以在 extraConfig 中手动添加 bind-tcp
    ...
    extraConfig = ''
      ...
      server 180.76.76.76 -group china -exclude-default-group # 百度 DNS

      # --- 2. 国外上游组 (group global) ---
      # 必须使用加密 DNS (DoT/DoH) 以穿透防火墙干扰
      # Cloudflare (DoT)
      server-tls 1.1.1.1 -group global -exclude-default-group
      server-tls 1.0.0.1 -group global -exclude-default-group
      
      # Google (DoT) - 8.8.8.8 在国内通常被阻断或严重丢包，
      # 建议作为备选，或者使用你的代理环境能访问的 DNS
      server-tls 8.8.4.4 -group global -exclude-default-group

      # Quad9 (DoT) - 在国内连接通常比 Google 稳
      server-tls 9.9.9.9 -group global -exclude-default-group
